...
namespace ToolBox.StateMachine
{
    ...
    public class StateMachineHistory<TStateMachine, TState, TStateContext> : StateMachineHistory
        ...
        where TStateContext : StateContext
    {
        ...
        public override void GoToState<TStateType>()
        {
            if (CurrentState?.Context == null)
            {
                throw new NullReferenceException(
                    "StateMachineHistory | GoToState<TStateType>() called before a context was sent");
            }

            Type stateType = typeof(TStateType);
            TStateContext context = CurrentState.Context;

            if (context.OpenState != stateType)
            {
                StateContext.UpdateOpenState(context, stateType);
            }

            GoToState(context);
        }

        public virtual void GoToState(TStateContext context)
        {
        ...
        public virtual void GoToStateNonHistorically(TStateContext context)
        {
        ...
        private void InternalStateTransition(TState nextState, TStateContext context)
        {
            InternalExitToEnterState(nextState, context);
        }
